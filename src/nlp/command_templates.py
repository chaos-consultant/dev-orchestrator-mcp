...
COMMAND_TEMPLATES = {
    ...
    r"list\s+folders?": "ls -d */",
    r"show\s+folders?": "ls -d */",
    r"tree\s+view": "tree -L 2",
    r"find\s+file.*named\s+(\S+)": "find . -name '{0}'",
    r"find\s+folder(?:s)?\s+(?:with\s+)?(?:named?\s+)?(\S+)": "find . -type d -name '*{0}*'",
    r"find\s+director(?:y|ies)\s+(?:with\s+)?(?:named?\s+)?(\S+)": "find . -type d -name '*{0}*'",
    r"list\s+folder(?:s)?\s+(?:with\s+)?.*?(\w+)": "find . -type d -name '*{0}*'",
    r"list\s+director(?:y|ies)\s+(?:with\s+)?.*?(\w+)": "find . -type d -name '*{0}*'",
